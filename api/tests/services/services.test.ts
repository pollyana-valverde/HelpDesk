...
describe('Services', () => {
  ...
  it('should create a service as admin', async () => {
    const response = await request(app)
      ...
      .send({ name: 'New Service', price: 150 })
  ...
  it('should update a service as admin', async () => {
    const createResponse = await request(app)
      .post('/services')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ name: 'Update Service', price: 100 })
  ...
  it('should not update a service as client', async () => {
    const createResponse = await request(app)
      .post('/services')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ name: 'Client Update Service', price: 100 })
    ...
    const response = await request(app)
      .put(`/services/${serviceId}/update`)
      .set('Authorization', `Bearer ${clientToken}`)
      .send({ name: 'Hacked Service', price: 999 })
